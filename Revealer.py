...
class ReceivePacket(events.EventEmitter):
    def __init__(self, interface):
        ...
        self.target_packets = ['%jr%', '%jz%', '%zm%']
        self.connected = False

    def start(self):
        def receive_packet(packet):
            packet_data = str(packet.load)
            is_xt_packet = '%xt' in packet_data
            if is_xt_packet and (not self.connected):
                self.connected = True
                self.interface.connected()
            if any(substring in packet_data for substring in self.target_packets):
...
class Revealer():
    ...
    def handle_zm(self, packet, deconstructed_packet):
        ...
        side = deconstructed_packet[5] # The penguins side that the packet came from
